...
parser.add_argument(
    "--scene_info",
    help="Path to the scene_info.json file. ",
    default=None,
)
...
if __name__ == "__main__":
    ...
    if args.scene_info is not None:
        with open(args.scene_info, "r") as f:
            scene_info = json.load(f)
        
        for test_name in ["first_segment", "second_segment", "remain_segments"]:
            benchmark(
                [args.gt_root],
                [args.pred_root],
                False,  # strict is not used in this case
                args.num_processes,
                verbose=False,
